...
async与then顺序，promise是同步的，then是异步的，then() 方法会返回一个新的 promise。
resolve() 必须等到其内部全部的异步 resolve() 执行完毕后才能够执行。
settimeout里resolve()？

* 在新建promise对象的回调函数里再定义一个promise对象，内部的promise的resolve/then永远先执行。
