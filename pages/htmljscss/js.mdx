# 高频
## var，let，const，死区？
var，let，const声明用法是一样，都是定义变量。
* var的声明会被提升到全局作用域或函数作用域，但仍然在原地方赋值。
* let，const并不会声明提升，因此在声明之前的代码区域就是暂时性死区。

## call，apply，bind的区别？
三者的功能都是用来改变函数中的 this 指向。
* call() 方法第一个参数代表函数内this的指向对象，从第二个参数开始往后都是作为函数参数传递给函数。 
* apply() 对比 call() 仅参数上的不同，第二个参数为一个列表，这个列表中的元素作为函数参数传递给函数。
* bind() 相对 call() 而言，仅绑定新对象并返回该函数，而不立即执行。

## 说一下原型和原型链？
原型是JavaScript对象的继承机制。JS所有对象都有一个私有属性proto指向另一个名为原型的对象，这个对象正是创建该实例的构造函数内的属性原型prototype。
...
> class继承的关系
首先，所有继承归根结底都是原型继承。
* 原型链继承，在构造函数外将子构造函数的原型对象prototype指向父对象实例实现继承。
但新实例无法向父类构造函数传参，继承单一，所有新实例都会共享父类实例的属性。
```js
...
```
* 构造函数内继承，在子构造函数内调用父构造函数的call函数绑定子对象并初始化，解决了原型链继承的问题。
但是完全没有用到原型，因此无法继承父类原型链上的属性。父类属性不复用的同时父类方法也不会复用。
* 组合继承，组合原型链继承和构造函数内继承。解决了原型链继承和构造函数继承的问题。
既能私有化父类对象的属性，又能继承父类原型上的属性。
但每次都会调用两次父类构造函数，子类原型上有一份多余的父类实例属性。
* 原型式继承，即Object.create(proto)，每次构造时都让prototype指向一个指定原型对象。
其原型链取决于构建对象时用的原型对象是什么。
* 寄生式继承，在原型继承的基础上再封装一次，给对象增加方法和属性。原型式看起开更像继承。
* 寄生式组合式继承，修改组合式继承的“子类原型指向父类实例”为“子类原型指向父类原型”，从而解决父类调用两次的缺陷，
但这时constructor会指向父类构造函数，因此最后还是需要将子类实例的constructor指回子类，这个问题可以用Object.create()可以解决。
...
## 事件循环机制？
JS是一门单线程的语言，事件循环是JS的异步执行机制。事件循环的工作流程如下：
* 首先，检查执行栈，看看是否有同步任务需要运行。
* 如果执行栈为空，那么就查看任务队列。
* 如果任务队列中有待处理的任务，那么就将它移出队列并放入调用堆栈并执行这个任务。

任务队列可以分为宏任务队列和微任务队列。在一个事件循环迭代中，首先会执行一个宏任务，然后执行所有的微任务。当所有的微任务完成后，再执行下一个宏任务。
...
* 捕获阶段：从 document 对象开始逐级向下传递，直到事件源元素。事件处理函数会按照由父元素到子元素的顺序被依次执行。
* 目标阶段：当事件传递到事件源元素时，就进入了目标阶段。在目标阶段可以通过 event.target 属性获取到具体触发事件的元素。
...
静态解析阶段分析依赖关系，输出符号引用。ES6模块JS文件相当于自带 defer属性，即异步加载JS文件，不阻塞DOM构建。
