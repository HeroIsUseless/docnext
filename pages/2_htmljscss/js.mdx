...
算是最简单的继承方式。但问题在于 1： 所有子类对象共享一个父类对象；2： 子类新实例无法向父类构造函数传参。
```js
function Child() {
  this.name = 'Mike';
}
...
const child = new Child();
```
* 拷贝式继承，每次构造时都拷贝一份原型对象，完全等价于Object.create。
...
```
* 拷贝组合式继承，组合式+拷贝式->拷贝组合式是一个循序渐进的过程。
通过借用构造函数来继承属性, 在原型上添加共用的方法, 
也就是说方法放到原型上，属性不放到原型上，最完美的继承方式。
...
## `for in`, `for of`区别？
* `for in`循环返回的值是对象的键值名（数组则是index，遍历顺序有可能不按照实际数组的内部顺序），
for… in会遍历所有的可枚举属性。所以不适合遍历数组，更适合遍历对象。
* for… of 循环返回的值是一对键值对中的值，for…of循环内部调用的实际上是数据结构的迭代器。因此不能遍历对象，因为普通对象没有迭代器，
